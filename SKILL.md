...
如平台无法直接生成或可靠检查 HTML、PDF 或 Excel，不得声称已经创建或交付失败文件。CSV、JSON、Sheet 结构、可复制表格与页面规范仅可作内部中间数据，不是正式交付。HTML 无法可靠生成或检查时停止文件交付；全流程 PDF 或任一 Excel、以及快速/详细档客户选择的 Excel 无法可靠完成时，按 `templates/html-delivery.md` 的统一降级规则只正式交付完整 HTML。
