...
- **快速**：确认已选择快速路线，进入快速路由入口。
- **详细**：确认已选择详细路线，进入详细路由入口。
- **全流程策划**：直接进入下述 2.8.0 完整规划流程。

本阶段只建立选择、映射和切换规则；快速与详细的沟通压缩在后续阶段实现。不得为临时效果复制三套 SOP，也不得声称快速或详细已经完成最终优化。全流程策划不得被削弱，继续使用原有完整流程。
...
客户已经在首次请求中明确档位时，直接记录对应档位，不重复询问时间问题：

- 快速或详细：确认已选择的路线后进入相应路由入口；本阶段不假装已经完成沟通压缩。
- 全流程策划：直接进入原有完整流程，可使用下列原有开场收集基本情况。
