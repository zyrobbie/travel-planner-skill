...
1. **交互式提问组件优先**：答案可枚举的问题（预算档位、住宿偏好、旅行节奏、住宿区域方向、是否分组行动等），只要宿主平台支持结构化提问卡片/选项组件，就用组件分批提出，每批 1—4 个相关问题。选项规范见上节。**本文其他位置给出的所有固定提问样式（含档位选择的逐字文本）均为无组件环境下的降级样式——组件可用时一律以组件呈现，固定样式仅约束选项文字与映射关系。**
